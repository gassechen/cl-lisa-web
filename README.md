...
[Video example](https://youtu.be/v_eEz3Z83uU?si=64bvK7y03946RqKM)
...
2.  **Clone Repository**:
    ...
    ```
3.  **Set up Lisa**: Clone the Lisa library into your Quicklisp local projects directory.
    ```sh
    cd ~/quicklisp/local-projects/
    git clone https://github.com/youngde811/Lisa.git
    ```
4.  **Load and Run**: In your Common Lisp REPL, load and start the system:
    ```lisp
    (ql:quickload :cl-lisa-web)
    (cl-lisa-web:start-server)
    ```
5.  **Access**: Open your web browser and navigate to `http://localhost:8888`.
